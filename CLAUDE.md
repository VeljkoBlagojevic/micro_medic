...
Every TS package (`design-system`, `design-system-react`, `design-system-angular`, `shared-types`, `shared-store`, `api-client`, `calendar`, `auth`, `examination`, `icd10`, `nav`, `notifications`) typechecks clean, `eslint .` is clean (zero errors *and* zero warnings), and all eight webpack packages build. Keep it that way — `yarn verify` is the single check.
...
| 3005 | `shared-store` | `shared_store` | `./store` | — (library) |
