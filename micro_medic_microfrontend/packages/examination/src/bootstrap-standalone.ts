...
// scope is initialized before any shared module is evaluated. `calendar` and `auth` do the same.
//
...
import('./standalone.js').catch((error: unknown) => {
    console.error('[examination] Failed to load the standalone entry point:', error);
});
